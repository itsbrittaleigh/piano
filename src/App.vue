...
export default {
  mounted() {
    const whiteKeys = document.querySelectorAll(".piano__white-key");
    whiteKeys.forEach((key, index) => {
      key.addEventListener("click", () => this.play(index + 1));
    });

    const blackKeys = document.querySelectorAll(".piano__black-key");
